...
{
    Table table = new ATab;
    table->arrarsize = size;
    table->data = new SetItem[size];
    table->last = 0;
    return table;
}

//判断成员存在
int TableMember(SetItem x, Table table)
{
...
//插入表
void TableInset(SetItem x, Table table)
{
    if (table->arrarsize == table->last)
        return;
    table->data[table->last++] = x;
}

//删除表
void TableDelete(SetItem x, Table table)
{
    bool isFind = false;
    for (int i = 0; i < table->last; i++)
    {
